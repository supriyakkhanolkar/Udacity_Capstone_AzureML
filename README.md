...
* Later we execute an Azure AutoML run using Python SDK for the same dataset to generates a number of machine learning models.
...
* Compare performance of best models generated by both the approaches and select a model for deployment.
* Deploy the selected model using notebook. It will be deployed as a web service in Azure Container Instance with authentication enabled.
* Create sample data for the web service. Use this data to send a HTTP Post request to the web service.
* Go to Endpoints tab of Azure ML Studio and locate the deployed web service. Note scoring URI and primary key of the service.
...
* C : Inverse of Regularization Strength. Regularization is used to apply a penalty to increase the magnitude of parameter values in order to reduce overfitting (default = 1.0)
* Max Iterations : Maximum number of iterations taken for the solvers to converge (default = 100)
...
### Training Script
Initially we created a training script train.py that takes Regularization Strength and Max Iterations as input. It does following tasks:
...
### Python Notebook
Then we created a notebook hyperparameter_tuning.ipynb to do following tasks:
...
### Notebook
We created a notebook automl.ipynb to do following tasks:
* Create compute cluster if not already present

![Screenshot](Screenshots/new_compute_cluster.jpg)

* Create and execute AutoML experiment using above mentioned configuration
...
![Screenshot](Screenshots/precision_recall.jpg)

#### ROC
![Screenshot](Screenshots/ROC.jpg)

#### Confusion Matrix
![Screenshot](Screenshots/confusion_matrix.jpg)

### Model Explantions
Azure ML generated explantions for the best model
#### Top-K Features

![Screenshot](Screenshots/explanations1.jpg)

#### Local explanations

![Screenshot](Screenshots/explanations2.jpg)

#### Dependency plots

![Screenshot](Screenshots/explanations3.jpg)
![Screenshot](Screenshots/explanations4.jpg)
![Screenshot](Screenshots/explanations5.jpg)

### Future Work
We can use the interpretability package of the Azure Machine Learning Python SDK to perform the following tasks:
  - Explain the entire model behavior or individual predictions
  - Enable interpretability techniques for engineered features
  - Use a visualization dashboard to interact with model explanations
  - Deploy a scoring explainer alongside model to observe explanations during inferencing
